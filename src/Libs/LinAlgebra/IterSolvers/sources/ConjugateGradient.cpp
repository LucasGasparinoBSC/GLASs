    ...
    double cgTime = this->IterSolvers_comm.timeFunction([&]
    {
        ...
        double zero_fp64 = static_cast<double>(0);
            ...
            while(this->iter < this->maxIters)
            {
                ...
                this->alpha[0] = this->resk[0] / this->alpha[0]; // alpha = resk/alpha

                // 3. Update x_sol = x_sol + alpha*p0
                TensorUtils<ITYPE, RTYPE>::axpy(this->arrSize, (RTYPE)this->alpha[0], this->p0, this->x_sol); // x_sol += alpha*p0
                ...
                this->aux[0] = this->beta[0]; // store new resk in aux
                this->beta[0] = this->beta[0] / this->resk[0]; // beta = resk_new / resk

                // 8. Update p0 = rk + beta*p0
                TensorUtils<ITYPE, RTYPE>::scale(this->arrSize, (RTYPE)this->beta[0], this->p0); // p0 = beta*p0
                TensorUtils<ITYPE, RTYPE>::axpy(this->arrSize, (RTYPE)1, this->rk, this->p0); // p0 += rk

                // 9. Update resk for next iteration
                this->resk[0] = this->aux[0]; // resk = aux (new resk)
...
void ConjugateGradient<ITYPE, RTYPE>::fpcgSolver(const MatVecOp& matvec, const PrecondOp& precond) {
    ...
    double cgTime = this->IterSolvers_comm.timeFunction([&]
    {
        ...
        RTYPE negOne = static_cast<RTYPE>(-1);
        double zero_fp64 = static_cast<double>(0);
        double negOne_fp64 = static_cast<double>(-1);
        #ifdef USE_GPU
            // Initial step
            PUSH_RANGE("fpcg init", 5);
            {
                // Copy x0 into x_sol
                DeviceUtils::launchKernel(copy_array<ITYPE, RTYPE>, this->kernelGrid, this->kernelBlock, this->kernelStream, this->d_x0, this->d_x_sol, this->arrSize); // x_sol = x0
                ...
                // res0 = tol*sqrt(resk) - convergence threshold
                DeviceMemory<ITYPE, double>::copyDeviceToHost(this->auxSize, this->d_resk, this->resk); // copy resk to host for convergence check
                this->res0[0] = this->tol * std::sqrt(this->resk[0]);
                POP_RANGE(); // 6
            ...
            while (this->iter < this->maxIters)
            {
                ...
                if (this->IterSolvers_comm.getLibSize() > 1)
                {
                    ...
                    DeviceUtils::launchKernel(copy_array<ITYPE, double>, this->auxGrid, this->auxBlock, this->kernelStream, this->d_mpiTmp, this->d_aux, this->auxSize); // aux = dot(rk+1,zk)
                    POP_RANGE(); // 8
                }
                POP_RANGE(); // 7
                ...
                precond(this->d_rk, this->d_zk); // zk+1 = M^-1 rk+1
                POP_RANGE(); // 7
                ...
                if (this->IterSolvers_comm.getLibSize() > 1)
                {
                    ...
                    DeviceUtils::launchKernel(set_array<ITYPE, double>, this->auxGrid, this->auxBlock, this->kernelStream, this->d_mpiTmp, zero_fp64, this->auxSize);
                    this->IterSolvers_comm.Allreduce_Sum(this->d_beta, this->d_mpiTmp, 1);
                    DeviceUtils::launchKernel(copy_array<ITYPE, double>, this->auxGrid, this->auxBlock, this->kernelStream, this->d_mpiTmp, this->d_beta, this->auxSize); // beta = dot(rk+1,zk+1)
                    POP_RANGE(); // 8
                }
                POP_RANGE(); // 7
                ...
                DeviceUtils::launchKernel(copy_array<ITYPE, double>, this->auxGrid, this->auxBlock, this->kernelStream, this->d_mpiTmp, this->d_resk, this->auxSize); // resk = mpiTmp (new resk)
                POP_RANGE(); // 7
