...
int main()
{
    ...
    float *x_d = DeviceMemory<uint32_t, float>::deviceCalloc(N);
    float *y_d = DeviceMemory<uint32_t, float>::deviceCalloc(N);
    ...
    // AXPY test
    {
        ...
        DeviceMemory<uint32_t, float>::copyHostToDevice(N, x_h, x_d);
        DeviceMemory<uint32_t, float>::copyHostToDevice(N, y_h, y_d);
        uint32_t blocks = (N + TILE_SIZE - 1) / TILE_SIZE;
        blocks = std::min(blocks, (uint32_t)MAX_BLOCKS);
        dim3 grid(blocks,1,1);
        dim3 block(TILE_SIZE,1,1);
        DeviceUtils::launchKernel(axpy<uint32_t,float>, grid, block, kStream, a, x_d, y_d, N);
        DeviceUtils::StreamSynchronize(kStream);
        DeviceMemory<uint32_t, float>::copyDeviceToHost(N, y_d, y_h);
        for (uint32_t i = 0; i < N; ++i) {
            if (y_h[i] != 3.0f) {
                std::cerr << "Error at index " << i << ": expected 3.0f, got " << y_h[i] << std::endl;
                exit(EXIT_FAILURE);
            }
        }
    ...
    // dot product test
    {
        ...
        for (uint32_t i = 0; i < N; ++i)
        {
            x_h[i] = static_cast<float>(i + 1); // 1.0f, 2.0f, ..., N*1.0f
            y_h[i] = 1.0f;
        }
        ...
        dim3 grid(blocks, 1, 1);
        dim3 block(TILE_SIZE, 1, 1);
        DeviceUtils::launchKernel(dot_product<uint32_t, float>, grid, block, kStream, x_d, y_d, res_d, N);
        DeviceUtils::StreamSynchronize(kStream);
        ...
        if (std::abs(*res_h - expected) > 1e-5) {
    ...
    // Scale test
    {
        ...
        dim3 grid(blocks, 1, 1);
        dim3 block(TILE_SIZE, 1, 1);
        DeviceUtils::launchKernel(scale<uint32_t, float>, grid, block, kStream, a, x_d, N);
        DeviceUtils::StreamSynchronize(kStream);
        DeviceMemory<uint32_t, float>::copyDeviceToHost(N, x_d, x_h);
        for (uint32_t i = 0; i < N; ++i)
        {
            if (x_h[i] != 2.0f) {
                std::cerr << "Error at index " << i << ": expected 2.0f, got " << x_h[i] << std::endl;
                exit(EXIT_FAILURE);
            }
        }
    ...
    // Copy array test
    {
        ...
        dim3 grid(blocks, 1, 1);
        dim3 block(TILE_SIZE, 1, 1);
        DeviceUtils::launchKernel(copy_array<uint32_t, float>, grid, block, kStream, x_d, y_d, N);
        DeviceUtils::StreamSynchronize(kStream);
        DeviceMemory<uint32_t, float>::copyDeviceToHost(N, y_d, y_h);
        for (uint32_t i = 0; i < N; ++i)
        {
