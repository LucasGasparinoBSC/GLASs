...
int main()
{
    ...
    double *x1 = (double *)calloc(narr, sizeof(double));
    double *x2 = (double *)calloc(narr, sizeof(double));
    for (uint32_t i = 0; i < narr; i++)
    {
        x1[i] = static_cast<double>(i + 1);
        x2[i] = static_cast<double>(2 * i);
    }
    POP_RANGE
    ...
    CUDA_CHECK(cudaMalloc((void **)&dx1, narr * sizeof(double)));
    CUDA_CHECK(cudaMalloc((void **)&dx2, narr * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(dx1, x1, narr * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(dx2, x2, narr * sizeof(double), cudaMemcpyHostToDevice));
    POP_RANGE
    ...
    launchKernel(dot_product<uint32_t, double>, grid, block, kstream, dx1, dx2, dres, narr);
    POP_RANGE
    ...
    PUSH_RANGE("AXPY", 0);
    launchKernel(axpy<uint32_t, double>, grid, block, kstream, a, dx1, dx2, narr);
    POP_RANGE
    PUSH_RANGE("Copy x2 D2H", 0);
    CUDA_CHECK(cudaMemcpy(x2, dx2, narr * sizeof(double), cudaMemcpyDeviceToHost));
    POP_RANGE
