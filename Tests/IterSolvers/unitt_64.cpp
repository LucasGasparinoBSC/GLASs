...
int main() {
    ...
    double* h_res = (double*)calloc(arrSize_l, sizeof(double));
    #if defined(USE_GPU)
        double* d_res = DeviceMemory<uint32_t,double>::deviceCalloc(arrSize_l);
        tSolv.getSolution(d_res);
        DeviceMemory<uint32_t,double>::copyDeviceToHost(arrSize_l, d_res, h_res);
    #else
